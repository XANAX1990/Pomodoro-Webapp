...
| **Star Rating** | แสดงอัตโนมัติหลังจบ Long Break หรือเมื่อกด Finish Session; เปิดเพิ่มเติมได้ผ่านปุ่ม FAB มุมล่างซ้าย บันทึกลง Firebase Firestore |
| **Micro-reward Toast** | ขึ้นข้อความกำลังใจสุ่มทุก 5 นาทีระหว่าง Pomodoro (สุ่มจาก 10 ข้อความ) หายเองใน 3.5 วินาที หรือกดปิดได้ เพื่อรักษาแรงจูงใจ |
...
| ฟีเจอร์ | รายละเอียด |
|--------|-----------|
| **Mini-Pomodoro** | ย่อระยะเวลาทำงานจากมาตรฐาน 25 นาที เหลือ 10–15 นาที ลดความกดดันในการเริ่มลงมือทำงาน (Task Initiation) และช่วยผู้มีภาวะตาบอดทางเวลา (Time Blindness) อ้างอิง Barkley (2014) [4] |
...
| **Pomodoro Movement** | เมื่อจบแต่ละรอบ จะขึ้น popup แนะนำท่าขยับร่างกาย (สุ่มจาก 7 ท่า เช่น Deep Breathing, ยืดแขน, หมุนคอ, พักสายตา) พร้อม ring countdown timer กด Done หรือ Skip ได้ อ้างอิงจากงานวิจัย Ratey & Hagerman (2008) ที่พบว่าการเคลื่อนไหวร่างกายช่วยกระตุ้น Dopamine และ Norepinephrine [5] |
...
| **Micro-reward Toast** | ระหว่าง Pomodoro ขึ้นข้อความกำลังใจสุ่มทุก 5 นาที (สุ่มจาก 10 ข้อความ) หายเองใน 3.5 วินาที หรือกดปิดได้ |
...
### 3.3.5 กระบวนการทำงานในรูปแบบ Flow Chart

แสดงผังกระบวนการดำเนินงานของการทำงานในหน้าเว็บไซต์ตามขอบเขตของปริญญานิพนธ์ เริ่มต้นจากการกำหนดและเลือกฟังก์ชันการทำงาน โดยมีรายละเอียดขั้นตอนดังนี้:

1. **จุดเริ่มต้น** — ระบบเริ่มทำงานเมื่อผู้ใช้เข้าสู่หน้าเว็บไซต์
2. **การเลือกฟังก์ชันการทำงาน** — ผู้ใช้เลือกโหมดผ่าน Navbar (Pomodoro / Short Break / Long Break)
3. **การประมวลผล** — กดปุ่ม START → JavaScript เริ่มฟังก์ชันนับถอยหลังตามเวลาที่กำหนด
4. **เงื่อนไขเวลา** — ระบบตรวจสอบว่าเวลาหมดหรือไม่ หากหมดจะเล่นไฟล์เสียงแจ้งเตือนเพื่อบอกสถานะแก่ผู้ใช้
